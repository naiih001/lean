...
pub(crate) fn cache_lock() -> &'static std::sync::Mutex<Option<(Vec<Skill>, Instant)>> {
    CACHE.get_or_init(|| std::sync::Mutex::new(None))
}

pub(crate) fn parse_frontmatter(raw: &str) -> (Option<String>, Option<String>, String) {
    if !raw.starts_with("---") {
...
pub(crate) async fn scan_dir(base: &Path) -> Vec<Skill> {
